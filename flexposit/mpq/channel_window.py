#!/usr/bin/env python3
# channel_window.py — apply FlexPosit mixed-precision quantization to a
# Posit(4,1) base checkpoint, guided by a per-channel-window sensitivity CSV.
#
# Modes (mutually exclusive):
#   --target_avg_bits X    Budget: upgrade top windows to hit target avg bits
#   --sweep_bits_start/end Sweep: eval PPL at each step across a bit range,
#                          write ppl_vs_avg_bits.csv (Pareto curve)
#   --ppl_goal P           PPL goal: upgrade greedily until wikitext PPL <= P
#   (none of the above)    Default: upgrade all windows with ΔPPL < 0
#
# Reads per-window rows from --sensitivity_csv (columns: layer, win_start,
# win_end, delta_ppl, ...; ascending sort selects highest-benefit windows).
...
def get_args():
    ...
    p.add_argument("--model", choices=list(MODEL_PRESETS.keys()), default=None,
                   help="Short model name; used to auto-derive --fp32_reference_dir "
                        "from MODEL_PRESETS when the latter is not given.")
    ...
    args = p.parse_args()
    if args.fp32_reference_dir is None:
        if args.model is None:
            p.error("Either --model or --fp32_reference_dir must be provided.")
...
def main():
    ...
    # Auto-cap --seqlen by the model's context window (gpt2 = 1024, most others 2048+).
    max_ctx = getattr(ref_model.config, "max_position_embeddings", None)
    if isinstance(max_ctx, int) and max_ctx > 0 and args.seqlen > max_ctx:
        print(f"[Info] --seqlen {args.seqlen} > model.max_position_embeddings {max_ctx}; "
